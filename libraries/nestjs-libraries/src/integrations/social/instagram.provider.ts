    ...
    try {
      ...
      if (jsonError?.error?.code === 352 && jsonError?.error?.error_subcode === 2207026) {
        ...
        if (firstPost.media && Array.isArray(firstPost.media)) {
          ...
          const validRetryPost = firstPost.media.find((m) => m.path.indexOf('.mp4') > -1 && !m.path.includes('-ig-publica-xxxxy.mp4'));
          ...
          if (validRetryPost) {
            return await this.post(id, accessToken, postDetails, integration, type);
          }
        }
      }
